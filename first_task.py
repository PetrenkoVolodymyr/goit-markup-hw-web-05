        ...
        async with aiohttp.ClientSession() as session:
            async with session.get('https://api.privatbank.ua/p24api/exchange_rates', params=params) as response:
                ...
                for currency in curr_list:
                    ...
                    currency_data_dor_date.append(one_curr_dict)
                return currency_data_dor_date


async def main():
    port = Request()
    currency_added = input("->")
    currency_list.append(currency_added)
    print(await port.handler(qty_days, currency_list))


if __name__ == "__main__":
    try:
        qty_days = min(10,int(sys.argv[1]))
    except:
        qty_days = 1
    asyncio.run(main())
